...
public class CalvinBridge {
    private static final String TAG = "AliuHook-XposedBridge";

    static {
        try {
            callbackMethod = CalvinBridge.HookInfo.class.getMethod("callback", Object[].class);
        } catch (Throwable t) {
            throw new RuntimeException("Failed to initialize", t);
        }

        System.loadLibrary("lspc");
    }
     ...
     */
    public static CA_MethodHook.Unhook hookMethod(Member method, CA_MethodHook callback) {
        checkMethod(method);
    ...
    public static Set<CA_MethodHook.Unhook> hookAllMethods(Class<?> hookClass, String methodName, CA_MethodHook callback) {
        Set<CA_MethodHook.Unhook> unhooks = new HashSet<>();
     ...
     */
    @SuppressWarnings("UnusedReturnValue")
    public static Set<CA_MethodHook.Unhook> hookAllConstructors(Class<?> hookClass, CA_MethodHook callback) {
        Set<CA_MethodHook.Unhook> unhooks = new HashSet<>();
     ...
     * @deprecated Use {@link CA_MethodHook.Unhook#unhook} instead. An instance of the {@code Unhook}
     * class is returned when you hook the method.
     */
    @SuppressWarnings("DeprecatedIsStillUsed")
    @Deprecated
    public static void unhookMethod(Member method, CA_MethodHook callback) {
        synchronized (hookRecords) {
     ...
     * the method and call {@code param.setResult(null)} in {@link CA_MethodHook#beforeHookedMethod}
     ...
     * {@link CalvinBridge#allocateInstance(Class)} in order to control when the constructor gets called.
     *
    ...
    public static class HookInfo {
        ...
        final CopyOnWriteSortedSet<CA_MethodHook> callbacks = new CopyOnWriteSortedSet<>();
        ...
        public Object callback(Object[] args) throws Throwable {
            var param = new CA_MethodHook.MethodHookParam();
            param.method = method;
            ...
            do {
                try {
                    ((CA_MethodHook) hooks[beforeIdx]).beforeHookedMethod(param);
                } catch (Throwable t) {
                    CalvinBridge.log(t);
            ...
            do {
                ...
                try {
                    ((CA_MethodHook) hooks[afterIdx]).afterHookedMethod(param);
                } catch (Throwable t) {
                    CalvinBridge.log(t);
...
//    public static
    public static Set<CA_MethodHook.Unhook> hookClassAllMethods(Class<?> hookClass, CA_MethodHook callback) {
        Set<CA_MethodHook.Unhook> unhooks = new HashSet<>();
        for (Member method : hookClass.getDeclaredMethods())
                unhooks.add(hookMethod(method, callback));
        return unhooks;
